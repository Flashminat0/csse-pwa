...
import {addAnimal, editAnimal, getAnimal} from "@/Api/animals"
...
const AnimalEdit = () => {
	...
	useEffect(() => {
		if (animal) {
			...
			setNameInput(animal.nameInput)
			setScientificNameInput(animal.scientificNameInput)
			setDescriptionInput(animal.descriptionInput)
		}
	}, [animal]);
	...
	const handleEditAnimal = async () => {
		...
		editAnimal({
			...
			await router.push(`/animals/${animal._id}`)
		}).catch(err => {
			toast.error(err)
		})
	}
